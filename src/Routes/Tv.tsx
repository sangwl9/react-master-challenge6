import { useMultipleTvQuery } from "../api";
...
const offset = 6;
type SectionId = "latest" | "airingToday" | "popular" | "topRated";
...
function Tv() {
    ...
    // Airing Today
    const [airingTodayDataIndex, setAiringTodayDataIndex] = useState(0);
    const [airingTodayDataLeaving, setAiringTodayDataLeaving] = useState(false);
    const increaseAiringTodayData = (data: any) => {
        if (data) {
            if (airingTodayDataLeaving) return;
            toggleAiringTodayDataLeaving();
            const totalMovies = data.results.length;
            const maxIndex = Math.floor(totalMovies / offset);
            setAiringTodayDataIndex((prev) =>
                prev === maxIndex ? 0 : prev + 1,
            );
        }
    };
    const toggleAiringTodayDataLeaving = () =>
        setAiringTodayDataLeaving((prev) => !prev);

    // Popular
    const [popularDataIndex, setPopularDataIndex] = useState(0);
    const [popularDataLeaving, setPopularDataLeaving] = useState(false);
    const increasePopularData = (data: any) => {
        if (data) {
            if (popularDataLeaving) return;
            togglePopularDataLeaving();
            const totalMovies = data.results.length;
            const maxIndex = Math.floor(totalMovies / offset);
            setPopularDataIndex((prev) => (prev === maxIndex ? 0 : prev + 1));
        }
    };
    const togglePopularDataLeaving = () =>
        setPopularDataLeaving((prev) => !prev);
    ...
    const clickedTv =
        ...
        allDataResult.find((tv) => tv.id === +bigTvMatch.params.tvId);
            ...
            loadingTopRated ? (
                ...
                <>
                    ...
                    <Slider>
                        <RightBtn
                            onClick={() =>
                                increaseAiringTodayData(airingTodayData)
                            }
                        >
                            {">"}
                        </RightBtn>
                        ...
                        <AnimatePresence
                            initial={false}
                            onExitComplete={toggleAiringTodayDataLeaving}
                        >
                            <Row
                                variants={rowVariants}
                                initial="hidden"
                                ...
                                key={makeRowKey(
                                    "airingToday",
                                    airingTodayDataIndex,
                                )}
                            >
                                {airingTodayData?.results
                                    .slice(
                                        offset * airingTodayDataIndex,
                                        offset * airingTodayDataIndex + offset,
                                    )
                                    .map((tv) => (
                                        <Box
                    ...
                    <Slider>
                        <RightBtn
                            onClick={() => increasePopularData(popularData)}
                        >
                            {">"}
                        </RightBtn>
                        ...
                        <AnimatePresence
                            initial={false}
                            onExitComplete={togglePopularDataLeaving}
                        >
                            <Row
                                variants={rowVariants}
                                initial="hidden"
                                ...
                                exit="exit"
                                transition={{ type: "tween", duration: 1 }}
                                key={makeRowKey("popular", popularDataIndex)}
                            >
                                {popularData?.results
                                    .slice(
                                        offset * popularDataIndex,
                                        offset * popularDataIndex + offset,
                                    )
                                    .map((tv) => (
                                        <Box
